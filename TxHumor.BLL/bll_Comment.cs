...
namespace TxHumor.BLL
{
    public class bll_Comment
    {
        ...
        /// <returns></returns>
        [Attr_CacheData("CacheCommentList")]
        public static List<int> GetComments(int pageIndex, int pageSize,out int recordCount)
        {
            DataTable dt = dal_Comment.GetComments(pageIndex, pageSize, out recordCount);
            if (dt == null || dt.Rows == null || dt.Rows.Count == 0) return null;
            List<int> ids = new List<int>(dt.Rows.Count);
            foreach (DataRow dr in dt.Rows)
            {
